...
At p=0.50 with Theta=0.05, fee per 100 contracts = 0.05 * 0.25 * 100 = $1.25.
At p=0.10 with Theta=0.05, fee per 100 contracts = 0.05 * 0.09 * 100 = $0.45.

Values are dollars per contract, where one contract pays $1 at resolution.
"""
...
def taker_fee_per_contract(price: float, theta_taker: float) -> float:
    ...
    return theta_taker * price * (1.0 - price)


def maker_rebate_per_contract(price: float, theta_maker: float) -> float:
    if not 0.0 < price < 1.0:
        raise ValueError(f"price must be in (0, 1); got {price}")
    return theta_maker * price * (1.0 - price)
